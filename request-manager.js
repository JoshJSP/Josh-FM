...
(()=>{
  const KEY='jfm_requests_v1',MAX_ACTIVE=8,MAX_ARMED_TRANSITIONS=3,MAX_NATURAL_MISSES=2,$=id=>document.getElementById(id);
  const now=()=>Date.now();
  let requests=load(),arming=false;
  const events=[];
  function load(){try{const x=JSON.parse(localStorage.getItem(KEY)||'[]');return Array.isArray(x)?x.filter(r=>r&&r.uri&&r.status!=='played').slice(0,MAX_ACTIVE).map(r=>({...r,armedTransitions:Number(r.armedTransitions||0),armAttempts:Number(r.armAttempts||0),natural:!!r.natural,naturalMisses:Number(r.naturalMisses||0)})):[]}catch{return[]}}
  ...
  function futurePosition(uri){const list=upcoming();const i=list.findIndex(t=>t?.uri===uri);return i>=0&&i<6?i+1:0}
  ...
  async function add(uri,button=null){
    ...
    const current=currentTrack();if(current?.uri===uri){status(`${current.name||'Dit nummer'} draait al op Josh FM.`);trace('request-already-playing',{uri});return null}
    ...
    const track=await resolveTrack(uri,button),naturalPos=futurePosition(uri),delay=naturalPos||chooseDelay(track),r={id:'req_'+now()+'_'+Math.random().toString(36).slice(2,7),uri,track,status:'planned',remaining:delay,requestedAt:now(),armedAt:0,armedTransitions:0,armAttempts:0,natural:!!naturalPos,naturalMisses:0};
    requests.push(r);save();paintButton(button,r);trace(naturalPos?'request-adopted-upcoming':'request-added',{uri,remaining:delay});status(naturalPos?`${track.name} stond al in de planning · request gekoppeld aan dat moment.`:`${track.name} staat gepland · verwacht over ongeveer ${delay} nummers.`);return r
  }
  ...
  function deviceQuery(){try{const id=window.JFMPlaybackState?.get?.()?.deviceId||window.JFMPlayback?.storedDevice?.()||localStorage.getItem('jfm_spotify_device_id')||'';return id?'&device_id='+encodeURIComponent(id):''}catch{return''}}
  async function armDue(){
    if(arming||requests.some(x=>x.status==='armed'))return false;const r=requests.find(x=>x.status==='planned'&&!x.natural&&Number(x.remaining)<=1);if(!r)return false;
    arming=true;try{
      const alreadyQueued=await remoteQueueHas(r.uri);if(!alreadyQueued)await api('/me/player/queue?uri='+encodeURIComponent(r.uri)+deviceQuery(),{method:'POST'});
  ...
  function onTrackChange(){
    ...
    if(played){played.status='played';played.playedAt=now();trace('request-played',{uri,natural:!!played.natural});requests=requests.filter(r=>r.id!==played.id);save();status(`${played.track?.name||'Request'} draait nu · request afgevinkt.`);setTimeout(()=>armDue().catch(()=>{}),300);return}
    for(const r of requests){
      if(r.status==='planned'){
        const before=Number(r.remaining)||1;r.remaining=Math.max(1,before-1);
        if(r.natural&&before<=1){r.naturalMisses=Number(r.naturalMisses||0)+1;if(r.naturalMisses>=MAX_NATURAL_MISSES){r.natural=false;r.naturalMisses=0;trace('natural-request-fallback',{uri:r.uri})}}
      }else if(r.status==='armed')r.armedTransitions=Number(r.armedTransitions||0)+1
    }
  ...
  window.JFMRequests={version:'requests-v3-natural-adoption',add,list:()=>requests.map(r=>({...r})),isRequest:t=>!!t&&requests.some(r=>(r.uri===t.uri||r.track?.id===t.id)&&(r.status==='planned'||r.status==='armed')),eta:t=>{const r=requests.find(x=>x.uri===t?.uri||x.track?.id===t?.id);return r?etaText(r):''},armDue,recoverStaleArmed,log:()=>[...events]};
})();
