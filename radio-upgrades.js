...
// Fish Audio and the emergency device fallback are both configured as English presenters.
try{
  ...
  if(typeof oldMake==='function'){
    window.makeDJScript=makeDJScript=async function(track,fact,weather,manual){
      ...
      const voiceLang=(mode==='fish'||mode==='device')?'en':((window.JFMDJLanguage||localStorage.getItem('jfm_dj_language')||'en').startsWith('nl')?'nl':'en');
      ...
      return oldMake(track,voiceLang==='en'?null:fact,weather,manual);
    };
  }
