...
(()=>{
  ...
  const readText=(k,d='')=>{try{return localStorage.getItem(k)||d}catch{return d}};
  ...
  function legacy(){
    ...
    return{settings,taste,onboarding,channel:readText('jfm_music_channel_v1','mix')}
  }
  function blank(){const l=legacy(),now=Date.now();return{
    schema:SCHEMA,revision:1,createdAt:now,updatedAt:now,
    activeUserId:'user-local',activeStationId:'station-josh-fm',
    users:{'user-local':{id:'user-local',displayName:'Josh',createdAt:now,localOnly:true}},
    stations:{'station-josh-fm':{id:'station-josh-fm',ownerId:'user-local',name:'Josh FM',language:'nl',musicProfileId:'music-default',djProfileId:'dj-default',historyId:'history-default',createdAt:now}},
    ...
    migrations:{legacyImportedAt:now,onboardingCompleted:!!l.onboarding?.completed}
  }}
