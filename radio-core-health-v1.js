...
(()=>{
  ...
  function sample(){
    ...
    const stalled=!!s.expectedLive&&!s.isPlaying&&!djBusy()&&!window.JFMPlayback?.health?.busy;
  ...
  window.addEventListener('offline',()=>push('offline'));
  document.addEventListener('visibilitychange',()=>push(document.hidden?'background':'foreground'));
  setInterval(sample,2000);setTimeout(sample,600);
  window.JFMRadioCoreHealth={version:'build1-health-v1',sample,events:()=>[...events],get state(){sample();return{...lastSample,deviceFlaps,rapidTransitions,stalls,eventCount:events.length}}};
})();