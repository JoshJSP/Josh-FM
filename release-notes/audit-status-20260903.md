...
| C-3 | Time Machine garandeert het jaarbereik niet in alle paden | **Niet meer van toepassing — feature verwijderd.** Time Machine is op 3 september 2026 als product verwijderd, samen met Chaos Mode en Afterparty. Het hele jaarvenster is daarmee weg: de eindpoort in `queue-core.js`, het filter in `rotation-engine.js`, de aanvulling in `station-queue.js`, de metadatacontrole in `mair-modes.js` en de bijbehorende poort `scripts/time-machine-gate-check.mjs`. Het punt hoeft niet meer opgelost te worden. |
...
| M-9 | Einde van een Time Machine-sessie onderbreekt de lopende track | **Niet meer van toepassing — feature verwijderd.** Er is geen Time Machine-sessie meer die kan aflopen. `stop()` in `mair-modes.js` bestaat nog voor de recap-laag, maar wordt niet meer door een modus aangeroepen. |
...
## Productbeslissing 3 september 2026 — drie modi verwijderd

Time Machine, Chaos Mode en Afterparty zijn op verzoek van Josh volledig uit het product
gehaald. Time Machine was daarvoor twee keer op de echte site gefaald; de andere twee werden
niet gebruikt.

Roadtrip, de vierde modus, was al eerder met pensioen gestuurd: `mair-journey-director.js`
verbergt de kaart en blokkeert het starten sinds commit `f8c783a` van 30 augustus, omdat de
journey director van Car Mode die rol heeft overgenomen. Na het verwijderen van de drie hield
het kopje "MAIR Modes" dus nul werkende modi over, en is het kopje zelf ook verdwenen:
`mair-modes-ui.js` en `mair-modes.css` zijn verwijderd.

`mair-modes.js` blijft wél bestaan. Dat bestand levert de recap- en analyticslaag waar **Your
Week on MAIR** op draait, plus `djContext` voor de DJ-contextbouwer en `rotationScore` voor de
rotatie. Alleen de mode-specifieke delen zijn eruit.
