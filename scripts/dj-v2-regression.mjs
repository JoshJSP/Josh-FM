...
ok(dj.includes("v3.4-deferred-breaks-memory-imaging"),'DJ v3.4 runtime marker missing');
ok(writer.includes('process.env.GROQ_API_KEY'),'Groq key must remain server-side');
ok(dj.includes("phase:'COUNTING'")&&dj.includes("'PREPARING'")&&dj.includes("'ARMED'")&&dj.includes("'HANDOFF'")&&dj.includes("'SPEAKING'")&&dj.includes("'RESTORING'"),'DJ v3.4 state machine incomplete');
...
ok(dj.includes("window.addEventListener('jfm:trackchange'")&&dj.includes('manual-or-unexpected-track-change')&&dj.includes('automatic-track-change-preserved'),'Generic track changes may cancel a manual request but must preserve automatic cadence');
ok(!dj.includes('schedulePendingRetry')&&!dj.includes('retryTimer')&&!dj.includes('retryAfter'),'DJ must never retry a failed on-air handoff on the same track');
ok(dj.includes('function deferBreak')&&dj.includes('automatic-break-deferred')&&!dj.includes("miss('automatic-break-not-ready')"),'Late preparation must defer to a future transition instead of dropping the scheduled break');
ok(dj.includes('function rearmOwed')&&dj.includes("rearmOwed(!!pack.manual")&&dj.includes("'break-missed'"),'Failed automatic handoff must remain owed for a future natural transition');
...
ok(dj.includes('pack.nextHintId&&currentId!==String(pack.nextHintId)'),'Stale next-track DJ copy must be rejected before speech');
...
ok(dj.includes('if(paused&&uri){try{await restoreMusic(uri,{rewind:false})}catch{}}'),'Failed voice handoff must restore Spotify once without rewind/same-track retry');
...
ok(primary.includes('primary-v13-sdk-confirmed-reload'),'Primary SDK-confirmed reload transport version missing');
...
if(fail.length){console.error('MAIR DJ v3.4 regression FAILED');for(const x of fail)console.error('-',x);process.exit(1)}console.log('MAIR DJ v3.4 regression OK');