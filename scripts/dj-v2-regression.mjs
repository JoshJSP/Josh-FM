...
const dj=read('mair-dj-v2.js'),writer=read('api/dj-writer.js'),boot=read('dj-now-queue.js'),voice=read('mair-voice-check.js'),unlock=read('mair-audio-unlock-v1.js'),diag=read('mair-diagnostics-hub.js'),easy=read('mair-easy-use-v1.js'),sw=read('sw.js'),legacy=read('dj-handoff-v34.js');
...
ok(diag.includes("document.addEventListener('click',toggleFromEvent,false)")&&diag.includes("body.style.display=open?'block':'none'")&&diag.includes("aria-expanded"),'Diagnose toggle must use delegated, explicit visible state handling');
