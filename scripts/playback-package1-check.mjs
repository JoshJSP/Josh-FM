...
async function testPrimarySingletonAndNaturalEnd(){
  ...
  const localStorage=storage({jfm_spotify_device_id:'device-1'}),metrics={intervals:0,play:0,transfer:0,pauseSdk:0,resumeSdk:0,previous:0,previousSdk:0},remote={item:{id:'A',uri:'spotify:track:AAAAAAAAAAAAAAAAAAAAAA'},device:{id:'device-1'},is_playing:false,progress_ms:0};let remoteVisible=true,sdkPlaying=false;
  const player={getCurrentState:async()=>({paused:!sdkPlaying,position:remote.progress_ms,track_window:{current_track:{uri:remote.item.uri}}}),activateElement(){},async pause(){metrics.pauseSdk++;sdkPlaying=false},async resume(){metrics.resumeSdk++;sdkPlaying=true},async previousTrack(){metrics.previousSdk++;remote.item={id:'A',uri:'spotify:track:AAAAAAAAAAAAAAAAAAAAAA'};remote.progress_ms=0}};
  const api=async(path,opt={})=>{if(path==='/me/player'&&opt.method==='PUT'){metrics.transfer++;remote.device={id:opt.body.device_ids[0]};remote.is_playing=!!opt.body.play;return null}if(path==='/me/player')return remoteVisible?structuredClone(remote):null;if(path.startsWith('/me/player/play?')){metrics.play++;remote.item={id:'B',uri:'spotify:track:BBBBBBBBBBBBBBBBBBBBBB'};remote.is_playing=true;remote.progress_ms=0;return null}if(path.startsWith('/me/player/previous?')){metrics.previous++;if(remote.progress_ms>3000)remote.progress_ms=0;else{remote.item={id:'A',uri:'spotify:track:AAAAAAAAAAAAAAAAAAAAAA'};remote.progress_ms=0}return null}throw Error(`Unexpected API ${path} ${opt.method||'GET'}`)};
  ...
  remote.item={id:'B',uri:'spotify:track:BBBBBBBBBBBBBBBBBBBBBB'};remote.progress_ms=42000;remote.is_playing=true;remote.device={id:'stale-page-device'};const playsBeforeRecover=metrics.play,transfersBeforeRecover=metrics.transfer,handoversBeforeRecover=context.JFMPlayback.health.deviceHandovers;await context.JFMPlayback.recover('reload-test');assert.equal(metrics.transfer,transfersBeforeRecover+1,'reload recovery must hand playback to the current browser SDK device');assert.equal(metrics.play,playsBeforeRecover,'reload recovery must not start a replacement station context');assert.equal(remote.item.id,'B','device handover must preserve the current track');assert.equal(remote.progress_ms,42000,'device handover must preserve playback position');assert.equal(remote.is_playing,true,'an active radio must remain playing through device handover');assert.equal(context.JFMPlayback.health.deviceHandovers,handoversBeforeRecover+1,'device handovers must be visible in diagnostics');
}
...
function testReloadedTruthRequiresFreshConfirmation(){
  const sessionStorage=storage({jfm_playback_truth_v1:JSON.stringify({trackId:'A',uri:'spotify:track:AAAAAAAAAAAAAAAAAAAAAA',isPlaying:true,expectedLive:true,operation:{id:4,type:'next',expiresAt:Date.now()+99999}})}),bus=new Events(),context={window:null,sessionStorage,CustomEvent:FakeCustomEvent,Date,console};Object.assign(context,{addEventListener:(...args)=>bus.addEventListener(...args),dispatchEvent:(...args)=>bus.dispatchEvent(...args)});context.window=context;vm.createContext(context);vm.runInContext(read('playback-state.js'),context,{filename:'playback-state.js'});const state=context.JFMPlaybackState.get();assert.equal(state.trackId,'A');assert.equal(state.expectedLive,true);assert.equal(state.isPlaying,false,'a reloaded page may not trust persisted audible playback');assert.equal(state.operation,null,'a reloaded page may not keep a stale transport lock');assert.equal(context.JFMPlaybackState.shouldRecover(),true)
}

function testTransientSdkErrorsHeal(){const source=read('stability-core.js'),primary=read('playback-primary.js');assert.ok(source.includes('playbackErrorTimer')&&source.includes("message('MAIR speelt.')"),'a transient SDK playback error must clear after confirmed playing state');assert.ok(source.includes('state?.isPlaying')&&source.includes('state.updatedAt'),'delayed playback errors must consult recent central playback truth');assert.ok(primary.includes("if(playing){if(state)ingest(state,'primary-resume-already')")&&primary.includes("info('Josh FM speelt.')"),'a confirmed already-playing state must replace a stale playback error');assert.ok(primary.includes('if(playing===null)return startDirect()'),'the visible play control must cold-start MAIR when Spotify has no active status');assert.ok(primary.includes('nudgeSdkPlayback')&&primary.includes('player()?.resume?.()'),'a Web API start must also nudge the audible browser SDK inside the user gesture');assert.ok(primary.includes('uris.includes(x.item?.uri)'),'Spotify may skip an unavailable first URI and confirm another playable track from the same station context');assert.ok(primary.includes('ensureActive({preserve:true})')&&primary.includes('deviceHandovers'),'reload recovery must preserve context while transferring to the current browser device')}
