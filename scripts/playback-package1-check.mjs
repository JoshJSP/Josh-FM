...
function testTransientSdkErrorsHeal(){const source=read('stability-core.js'),primary=read('playback-primary.js');assert.ok(source.includes('playbackErrorTimer')&&source.includes("message('MAIR speelt.')"),'a transient SDK playback error must clear after confirmed playing state');assert.ok(source.includes('state?.isPlaying')&&source.includes('state.updatedAt'),'delayed playback errors must consult recent central playback truth');assert.ok(primary.includes('if(playing===null)return startDirect()'),'the visible play control must cold-start MAIR when Spotify has no active status');assert.ok(primary.includes('uris.includes(x.item?.uri)'),'Spotify may skip an unavailable first URI and confirm another playable track from the same station context')}
