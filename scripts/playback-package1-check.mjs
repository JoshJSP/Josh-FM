...
async function testPrimarySingletonAndNaturalEnd(){
  ...
  const localStorage=storage({jfm_spotify_device_id:'device-1'}),sessionStorage=storage({jfm_playback_truth_v1:JSON.stringify({trackId:'B',uri:'spotify:track:BBBBBBBBBBBBBBBBBBBBBB',progressMs:42000,durationMs:240000,isPlaying:true,expectedLive:true,deviceId:'stale-page-device',updatedAt:Date.now()})}),metrics={intervals:0,play:0,transfer:0,pauseSdk:0,resumeSdk:0,previous:0,previousSdk:0},remote={item:{id:'A',uri:'spotify:track:AAAAAAAAAAAAAAAAAAAAAA'},device:{id:'device-1'},is_playing:false,progress_ms:0};let remoteVisible=true,sdkPlaying=false;
  ...
  remote.item={id:'C',uri:'spotify:track:CCCCCCCCCCCCCCCCCCCCCC'};remote.progress_ms=0;remote.is_playing=false;remote.device={id:'stale-page-device'};const playsBeforeRecover=metrics.play,transfersBeforeRecover=metrics.transfer,handoversBeforeRecover=context.JFMPlayback.health.deviceHandovers;await context.JFMPlayback.recover('reload-test');assert.equal(metrics.transfer,transfersBeforeRecover+1,'reload recovery must hand playback to the current browser SDK device');assert.equal(metrics.play,playsBeforeRecover+1,'a stale paused SDK context must be replaced by the persisted live track');assert.equal(remote.item.id,'B','reload recovery must restore the intended track');assert.ok(remote.progress_ms>=42000,'reload recovery must preserve or advance playback position');assert.equal(remote.is_playing,true,'an active radio must remain playing through exact reload restore');assert.equal(context.JFMPlayback.health.deviceHandovers,handoversBeforeRecover+1,'device handovers must be visible in diagnostics');assert.equal(context.JFMPlayback.health.reloadRestores,1,'exact reload restores must be visible in diagnostics');
}
