...
function testStaticContracts(){
  const suite=read('radio-suite.js'),station=read('station-queue.js'),requests=read('request-manager.js'),director=read('director.js'),truth=read('spotify-upcoming-truth.js'),sw=read('sw.js'),build7=read('build7.js');
  assert.ok(suite.indexOf("'./queue-core.js'")<suite.indexOf("'./request-manager.js'"));assert.ok(suite.includes("'./spotify-upcoming-truth.js'"));assert.ok(station.includes("JFMQueue.buildActive('continuity-")&&station.includes('MAIR programmeert vooruit'));assert.ok(!station.includes('appended.has(track.id)||remote.has'));assert.ok(requests.includes('JFMQueue.programNext')&&!requests.includes("/me/player/queue?uri="));assert.ok(build7.includes('window.JFMRequests.add(d.uri,b)')&&build7.includes("'mair:request-confirmed'")&&build7.includes('requestConfirmCapture'));assert.ok(suite.includes("'mair:request-confirmed'"));assert.ok(director.includes('if(window.JFMSpotifyUpcomingTruth)')&&director.includes('function renderNext(){paintNext();if(!window.JFMSpotifyUpcomingTruth)'));assert.ok(truth.includes('v2-single-authoritative-owner'));assert.ok(sw.includes('mair-v73-request-delegated-20260824')&&sw.includes("CORE.push('./queue-core.js','./spotify-upcoming-truth.js')"))
}
