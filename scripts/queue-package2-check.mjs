...
async function testPlayedRequestAnnouncementMarker(){
  ...
  function boot(current){
    ...
    const document={readyState:'complete',visibilityState:'visible',getElementById:id=>id==='queueInfo'?queueInfo:null,addEventListener:()=>{}};
    const context={window:null,document,localStorage:data,CustomEvent:FakeCustomEvent,Date,Promise,console,Math:Object.create(Math),setInterval:()=>0,setTimeout:(fn)=>{Promise.resolve().then(fn);return 0},clearTimeout:()=>{},playback:{item:current},trackObj:t=>({id:t.id,uri:t.uri,name:t.name,artists:(t.artists||[]).map(a=>a.name||a)}),api:async path=>path.startsWith('/tracks/')?track(9,'Request Artist'):null,JFMQueue:{programNext:async()=>true}};
    context.Math.random=()=>0;Object.assign(context,{addEventListener:(type,fn)=>{const a=listeners.get(type)||[];a.push(fn);listeners.set(type,a)},dispatchEvent:e=>{for(const fn of listeners.get(e.type)||[])fn(e);return true}});context.window=context;vm.createContext(context);vm.runInContext(read('request-manager.js'),context,{filename:'request-manager.js'});return context;
  }
  const firstTrack=track(1,'Artist 1'),requested=track(9,'Request Artist'),first=boot({id:firstTrack.id,uri:firstTrack.uri,name:firstTrack.name,artists:[{name:'Artist 1'}]});
  await first.JFMRequests.add(requested.uri);first.playback.item={id:requested.id,uri:requested.uri,name:requested.name,artists:[{name:'Request Artist'}]};first.dispatchEvent(new FakeCustomEvent('jfm:trackchange'));await sleep(0);
  assert.equal(first.JFMRequests.list().length,0,'played request must leave active request queue');assert.equal(first.JFMRequests.isRequest(requested),true,'played current request must remain identifiable');const marker=first.JFMRequests.currentRequest(requested);assert.ok(marker,'played request marker must exist');assert.equal(marker.uri,requested.uri);assert.ok(marker.requestId);assert.ok(marker.playedAt);
  const reloaded=boot({id:requested.id,uri:requested.uri,name:requested.name,artists:[{name:'Request Artist'}]});assert.equal(reloaded.JFMRequests.isRequest(requested),true,'played request marker must survive reload');assert.ok(reloaded.JFMRequests.currentRequest(requested),'marker must be readable after reload');const consumed=reloaded.JFMRequests.consumeCurrentRequest(requested);assert.ok(consumed,'marker must be consumable once');assert.equal(consumed.requestId,marker.requestId);assert.equal(reloaded.JFMRequests.consumeCurrentRequest(requested),null,'marker must not be consumable twice');assert.equal(reloaded.JFMRequests.isRequest(requested),false,'consumed marker must no longer identify track as request');
  data.setItem('jfm_played_request_v1',JSON.stringify({requestId:'stale-request',uri:requested.uri,trackId:requested.id,name:requested.name,artists:['Request Artist'],playedAt:Date.now()-(11*60*1000)}));const stale=boot({id:requested.id,uri:requested.uri,name:requested.name,artists:[{name:'Request Artist'}]});assert.equal(stale.JFMRequests.currentRequest(requested),null,'expired marker must be ignored');assert.equal(data.getItem('jfm_played_request_v1'),null,'expired marker must be deleted');
}
