...
const oldDjOwner="ok('DJ v36.2 owner',dj.includes(\"dataset.jfmHandoffOwner='v36'\")&&dj.includes('handoff-v36.2-mute-rewind-skip-safe'));";
const newDjOwner="ok('DJ handoff is transport-only',dj.includes('handoff-v37.1-voice-guard')&&!dj.includes('dataset.jfmHandoffOwner')&&!dj.includes('consumeArmedIfChanged')&&read('dj-authoritative-v226.js').includes('v230.1-event-driven-single-owner'));";
if(patched.includes(oldDjOwner))patched=patched.replace(oldDjOwner,newDjOwner);
const oldDjManual="ok('DJ now survives manual skip',dj.includes('consumeArmedIfChanged')&&dj.includes('armed={fromId:id,requestedAt:Date.now()}')&&dj.includes('runBreak(null,true)')&&dj.includes(\"window.addEventListener('jfm:trackchange'\"));";
const newDjManual="ok('DJ manual scheduling owned by authoritative scheduler',read('dj-authoritative-v226.js').includes(\"dataset.mairDjOwner='authoritative'\")&&read('dj-authoritative-v226.js').includes('armedFrom')&&read('dj-authoritative-v226.js').includes('await run(true)')&&read('dj-authoritative-v226.js').includes(\"window.addEventListener('jfm:trackchange'\"));";
if(patched.includes(oldDjManual))patched=patched.replace(oldDjManual,newDjManual);
