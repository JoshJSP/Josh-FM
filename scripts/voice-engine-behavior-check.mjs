...
const legacySpeak=async(text,_jingle,meta={})=>{metrics.speak++;if(text.includes('zonder bewijs'))return true;const playbackStartedAt=Date.now();meta.onPlaybackStart?.({breakId:meta.breakId,provider:'fish',route:'test-audio',playbackStartedAt,success:true});return new Promise(resolve=>{const timer=setTimeout(()=>{cancelCurrent=null;meta.onPlaybackEnd?.({breakId:meta.breakId,provider:'fish',route:'test-audio',playbackStartedAt,playbackEndedAt:Date.now(),success:true});resolve(true)},35);cancelCurrent=()=>{clearTimeout(timer);cancelCurrent=null;meta.onPlaybackEnd?.({breakId:meta.breakId,provider:'fish',route:'test-audio',playbackStartedAt,playbackEndedAt:Date.now(),success:false,error:'cancelled'});resolve(false)}})};
...
const correlated=metrics.events.filter(e=>e.type==='mair:dj-speaking'&&e.detail.breakId==='cancel-me');assert.equal(correlated.length,2);assert.ok(correlated[0].detail.playbackStartedAt>0);assert.ok(correlated[1].detail.playbackEndedAt>=correlated[0].detail.playbackStartedAt);
const before=metrics.events.length;assert.equal(await window.speakText('Deze provider zegt klaar zonder bewijs.',false,{breakId:'missing-evidence'}),false,'provider success without a real playback callback must fail');assert.equal(metrics.events.length,before,'missing playback evidence may not synthesize an on-air event');
