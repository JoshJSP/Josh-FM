...
const checks=[
 ...
 ['dj-single-attempt-failsafe',dj.includes('lastNaturalSig')&&dj.includes('rearmOwed(!!pack.manual')&&dj.includes('restoreMusic(uri,{rewind:false})')&&!dj.includes('retryTimer')],
 ['dj-auto-break-remains-owed',dj.includes('function rearmOwed')&&dj.includes('count=Math.max(count,target)')&&dj.includes('automatic-track-change-preserved')],
