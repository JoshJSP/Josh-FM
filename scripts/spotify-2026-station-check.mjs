...
ok(controller.includes("mair_station_pool_cache_v5_nl_editorial_strict"),'station pools must use a fresh cache after Spotify API migration, Dutch chart migration and strict Nederlandstalig source migration');
...
ok(policy.includes("hard:t=>releaseYear(t)>=year()-2")&&controller.includes("if(id==='new')return list.filter(t=>yr(t)===now)")&&discovery.includes('y>=NOW()-1'),'Discovery must remain bounded to recent releases');
