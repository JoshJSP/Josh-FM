...
class Bus{constructor(){this.listeners=new Map()}addEventListener(name,fn){const list=this.listeners.get(name)||[];list.push(fn);this.listeners.set(name,list)}dispatch(name){for(const fn of this.listeners.get(name)||[])fn({type:name})}}
...
function harness({expectedLive=true,isPlaying=true,remotePlaying=false,recoverResult=true}={}){
  ...
  const window={addEventListener:(...args)=>winBus.addEventListener(...args),dispatchEvent:event=>{events.push(event.detail);return true},JFMPlaybackState:{get:()=>({...state}),setExpectedLive:on=>{metrics.expected++;state.expectedLive=!!on},ingest:live=>{metrics.ingest++;state.isPlaying=!!live.is_playing}},JFMPlayback:{recover:async()=>{metrics.recover++;return recoverResult}},JFMPWA:{reassertMediaSession(){}},navigator:{mediaSession:{}},document};
  ...
  return{window,document,events,metrics,hide(){document.visibilityState='hidden';docBus.dispatch('visibilitychange')},show(){document.visibilityState='visible';docBus.dispatch('visibilitychange')},pagehide(){winBus.dispatch('pagehide')}}
}
...
const tests=[['background return keeps live playback',stillPlayingNeedsNoRecovery],['foreground recovery is single-flight',pausedForegroundRecoversOnce],['inactive session never autostarts',inactiveSessionDoesNotAutostart]];
