...
expect(manifest.includes('mair-icon-192.png')&&manifest.includes('mair-icon-512.png')&&fs.statSync('apple-touch-icon.png').size>1000,'PWA/iPhone PNG-iconen ontbreken');
expect(sw.includes("'./apple-touch-icon.png'")&&sw.includes("'./mair-icon-192.png'")&&sw.includes("'./mair-icon-512.png'"),'PWA cachet de installatie-iconen niet');
...
expect(brain.includes("maxHour=talk==='LESS'?2:talk==='MORE'?5:4")&&brain.includes("threshold=talk==='LESS'?7:talk==='MORE'?4:6"),'Radio Brain praatfrequentie of uurcap is onverwacht gewijzigd');
...
expect(writer.includes('persoonlijke Nederlandse muziekzender')&&writer.includes('natuurlijk hedendaags Nederlands'),'Groq DJ Writer is niet strikt Nederlands');
...
expect(dj.includes('const ok=await window.speakText(pack.text,false,{breakId:pack.breakId,signal:pack.voiceController?.signal})')&&dj.includes("setPhase('SPEAKING'")&&!dj.includes('const audio=new Audio()'),'Hoorbare DJ-completion moet via de centrale Voice Engine lopen');
...
expect(dj.includes("window.addEventListener('mair:track-transition'")&&dj.includes("detail.cause==='NATURAL_END'")&&dj.includes('async function naturalTransition')&&!dj.includes('setInterval('),'Automatische DJ scheduler moet uitsluitend door canoniek bewezen natuurlijke trackovergangen worden gedreven');
...
expect(primary.includes('if(backgrounded()||reloadNeedsGesture)return false'),'Playback recovery mag niet in de achtergrond of vóór de vereiste gebruikersinteractie vechten');
...
expect(voiceCheck.includes("dataset.diagnosticScope='audible-voice-chain'"),'Voice Check is niet exclusief als diagnostic gemarkeerd');
expect(sw.includes("const CACHE='mair-v130-public-dj-enabled-diagnostics-isolated-20260830'"),'service-worker cache is niet de huidige MAIR-release');
