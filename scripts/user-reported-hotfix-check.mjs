...
assert.match(dj,/v3\.4-deferred-breaks-memory-imaging/);assert.match(dj,/function deferBreak/);assert.match(dj,/function rearmOwed/);assert.match(dj,/automatic-break-deferred/);assert.match(dj,/automatic-track-change-preserved/);assert.doesNotMatch(dj,/miss\('automatic-break-not-ready'\)/);assert.match(dj,/writer-memory-imaging-handoff-v4-auto-schedule-preserve/);
...
assert.match(diagnostics,/moveCard\('mairStationDirectorCard','STATION DIRECTOR'\)/);assert.match(diagnostics,/moveCard\('mairVoiceEngineCard','VOICE ENGINE'\)/);assert.match(diagnostics,/moveControl\('mairImagingPreview','SONIC LOGO TEST'\)/);assert.match(diagnostics,/VOICE LAB/);assert.match(diagnostics,/LANGE-DUURTEST/);
...
assert.match(version,/mair-sleep\.js/);assert.match(version,/mair-news-bulletin\.js/);assert.match(version,/mair-dj-break-owed-guard\.js/);assert.match(version,/JFM_ASSET_VERSION='78'/);assert.match(sw,/mair-v92-hourly-news-20260826/);assert.match(sw,/\.\/mair-sleep\.js/);assert.match(sw,/\.\/mair-dj-break-owed-guard\.js/);
console.log('MAIR user-reported hotfix gate: PASS — automatic DJ cadence, Sleep acoustic station/cover, settings cleanup, diagnostics centralization, HITS repeat lock and Spotify rate-limit mitigation');