...
const app=read('app.js');const env=read('.env.example');let checks=0;const ok=(name,value)=>{checks++;if(!value)throw new Error(`FAIL: ${name}`)};
ok('one semantic RadioViewState owner',state.includes("version:'radio-view-state-v2-auth-device'")&&state.includes('playbackPendingAction')&&state.includes('recoverableError'));
ok('public DJ exposes human radio phases without technical generation states',['LISTENING','SCHEDULING','SILENT','PREPARING','ON_AIR','RECOVERING'].every(x=>state.includes(`state:'${x}'`))&&!state.includes("state:'GENERATING'"));
ok('normal DJ state exposes countdown copy',state.includes('Nog ongeveer ${remaining} nummers')&&state.includes('Radiomoment komt dichtbij'));
ok('Spotify connection is based on auth rather than device DOM',state.includes('authenticated:!!(hasRefresh||hasAccess)')&&state.includes("label:a.reauthRequired?'Opnieuw koppelen nodig'")&&!state.includes("status?.classList.contains('on')||window.JFMSpotifySDK?.deviceId"));
ok('only definitive reauth state offers reconnect',state.includes("diagnosticsCode:'SPOTIFY_REAUTH_REQUIRED'")&&state.includes("primaryAction:'reconnect'"));
ok('device and gesture recovery never suggest OAuth',state.includes("diagnosticsCode:'SPOTIFY_DEVICE'")&&state.includes("diagnosticsCode:'AUDIO_GESTURE'")&&state.includes("primaryAction:'device'")&&state.includes("secondaryAction:'diagnostics'"));
...
ok('blocking errors expose recovery actions',ux.includes('data-mairfm-error-action')&&state.includes("primaryAction:'device'")&&state.includes("secondaryAction:'diagnostics'"));
...
console.log(`PASS UX contract v2: ${checks} checks`);
