...
function recoverMissedBreak(d){const missAt=Number(d?.lastMissAt||0);if(!missAt||missAt===lastRecoveredMissAt||d?.lastMissReason!=='break-missed')return false;lastRecoveredMissAt=missAt;lastReason='air-failure-rearmed';schedule(()=>{if(document.visibilityState==='hidden'||window.MAIRDJ?.busy)return;const id=currentTrackId();if(!id)return;if(window.MAIRDJ?.armManual?.()){recoveries++;emit()}},RECOVERY_DELAY);return true}
