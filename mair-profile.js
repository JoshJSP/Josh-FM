...
const $=id=>document.getElementById(id),PROFILE_KEY='mair_profile_v1',MODE_DATA_KEY='mair_mode_analytics_v1',DISC_KEY='jfm_discovered_tracks_v1',DAY=86400000;
...
// Alleen de week bestaat nog; Month en Year zijn uit het product gehaald.
function periodStart(now=Date.now()){const d=new Date(now),day=(d.getDay()+6)%7;d.setHours(0,0,0,0);d.setDate(d.getDate()-day);return d.getTime()}
function trackCounts(){const events=modeAnalytics(),tracks=events.filter(x=>x&&x.type==='track');if(!tracks.length)return null;const start=periodStart(),now=Date.now();return{total:tracks.length,period:tracks.filter(x=>Number(x.at)>=start&&Number(x.at)<=now).length}}
function periodDiscoveryCount(){const start=periodStart(),now=Date.now();return discoveryHistory().filter(x=>Number(x.at)>=start&&Number(x.at)<=now).length}
...
function wipePersonalStorage(storage){const preserve=new Set([PROFILE_KEY,'jfm_discovery']);const exact=new Set(['jfm_radio_suite',DISC_KEY,MODE_DATA_KEY,'jfm_top40_telemetry_v1','jfm_top40_snapshot_v1','jfm_taste_model_v4','jfm_director_memory','jfm_skips','jfm_hit_battle_v1','jfm_requests_v1','jfm_played_request_v1','jfm_dj_feedback','jfm_discovery_diag_v6','mair_profile_recap_period_v1','mair_profile_roast_round_v1']);const personal=/(telemetry|analytics|taste|memory|skip|top40|hit_battle|discover|request|feedback|recap|roast|history|recent|favorite|favourite|like|ban|learn)/i;for(let i=storage.length-1;i>=0;i--){const key=storage.key(i);if(!key||preserve.has(key))continue;if(exact.has(key)||personal.test(key))storage.removeItem(key)}}
function resetListeningProfile(){try{window.JFMTop40?.clear?.()}catch{}try{window.JFMTasteModel?.reset?.()}catch{}try{window.JFMRadioSuite?.resetDiscoveries?.()}catch{}try{wipePersonalStorage(localStorage);wipePersonalStorage(sessionStorage)}catch{}try{const s=window.JFMRadioSuite?.state?.();if(s){Object.assign(s,{minutes:0,tracks:0,discoveries:0,requests:0,likes:0,dislikes:0,lastIds:[],lastArtists:[],startedAt:Date.now()});window.JFMRadioSuite?.save?.(s)}}catch{}try{for(const key of [MODE_DATA_KEY,'jfm_top40_telemetry_v1','jfm_top40_snapshot_v1','jfm_director_memory','jfm_skips','jfm_hit_battle_v1','jfm_requests_v1','jfm_played_request_v1','jfm_taste_model_v4',DISC_KEY,'mair_profile_recap_period_v1','mair_profile_roast_round_v1'])localStorage.removeItem(key)}catch{}try{window.dispatchEvent(new CustomEvent('mair:profile-reset',{detail:{at:Date.now()}}))}catch{}setTimeout(()=>location.reload(),120);return true}
function render(){ensureCss();const pane=$('tab-settings');if(!pane)return null;pane.classList.add('mair-profile-tab');$('mairfmSettingsShortcut')?.remove();let page=$('mairProfilePage');if(!page){page=document.createElement('div');page.id='mairProfilePage';pane.appendChild(page)}const p=profile(),s=suite(),f=favoriteData(),w=recap('weekly'),periodDiscoveries=periodDiscoveryCount(),counts=trackCounts(),minutes=Number(s.minutes||0),totalTracks=counts?counts.total:Number(s.tracks||0),periodTracks=counts?counts.period:Number(w.tracks||0);const favArtists=f.topArtists.length?f.topArtists.join(', '):'Nog in opbouw';const favoritePeriod='Wordt geleerd door MAIRFM';const mood=dna(s);const recapMinutes=Number(w.minutes||0),recapTitle=recapMinutes?`${duration(recapMinutes)} deze week`:'Je week is in opbouw';page.innerHTML=`
...
<section class="mair-profile-card mair-profile-week">
  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:13px"><div class="mair-profile-section-title" style="margin-bottom:0;min-width:0"><i>◫</i><span><b>Your Week on MAIR</b><small>${esc(recapTitle)}</small></span></div></div>
...
<section class="mair-profile-card"><div class="mair-profile-section-title"><i>♪</i><span><b>Geluid &amp; DJ</b><small>Volume en de live DJ</small></span></div><div style="padding:2px 2px 12px"><div class="mair-profile-goal-head" style="margin-bottom:8px"><b>MAIR-volume</b><span id="volumeValue">100%</span></div><input id="volume" type="range" min="0" max="100" value="100" step="1" aria-label="MAIR-volume" style="width:100%"><small style="display:block;color:#8f9099;margin-top:6px">Los van de volumeknoppen van je telefoon.</small></div><div class="mair-profile-list"><div class="mair-profile-row mair-profile-row-action" data-mair-dj-toggle role="button" tabindex="0"><span class="mair-profile-row-icon">🎙️</span><span><b>Live DJ</b><small data-mair-dj-hint>Praat tussen de nummers door. Tik om hem op dit toestel uit te zetten.</small></span><em data-mair-dj-state aria-hidden="true">UIT</em></div></div></section><section class="mair-profile-card mair-profile-account"><div class="mair-profile-section-title"><i>●</i><span><b>App & account</b><small>Spotify, privacy en ondersteuning</small></span></div><div class="mair-profile-list">${row('◉','Spotify',spotifyConnected()?'Gekoppeld aan MAIRFM':'Niet gekoppeld')}${row('⌁','Privacy','Luisterdata blijft lokaal op dit apparaat')}${row('◷','Versie',releaseLine())}${row('⇧','Backup exporteren','Bewaar je MAIR-gegevens als bestand','backupExport')}${row('⇩','Backup importeren','Zet een eerder bewaard bestand terug','backupImport')}${row('◇','Geavanceerd & diagnostiek','Technische status, tests en herstel','diagnostics')}${row('↺','Alles resetten','Wis alle MAIR-luisterdata en begin opnieuw','resetProfile')}${row('↪','Spotify ontkoppelen','Verbreek de huidige Spotify-sessie','logout')}</div></section>
...
try{window.JFMPlayback?.bindVolume?.()}catch{}try{window.MAIRDiagnosticsHub?.paintDjSwitch?.()}catch{}
  page.querySelector('[data-mair-dj-toggle]')?.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.target.click()}});
...
window.MAIRProfile={version:'mair-profile-v13-week-only-recap',render,profile,saveProfile,resetListeningProfile,spotifyConnected};
})();
