// Josh FM release identity. Technical package version remains semver-compatible; releaseLabel is user-facing.
window.JFM_RELEASE={version:'2.2.5',releaseLabel:'v2b.0.0',build:'unknown',asset:'40',localCache:'unknown',serverCache:'unknown',updateAvailable:false};
...
(()=>{
  ...
  sanitizeSleepState();
  const render=()=>{const version=document.getElementById('appVersion'),build=document.getElementById('appBuild');if(version)version.textContent='Josh FM · '+(window.JFM_RELEASE.releaseLabel||('v'+window.JFM_RELEASE.version));if(build)build.textContent='Build '+window.JFM_RELEASE.build};
  ...
  async function resolveBuild(){render();try{const r=await fetch('/api/version',{cache:'no-store',headers:{accept:'application/json'}});if(r.ok){const data=await r.json();if(data?.version)window.JFM_RELEASE.version=String(data.version);if(data?.releaseLabel)window.JFM_RELEASE.releaseLabel=String(data.releaseLabel);if(data?.commit)window.JFM_RELEASE.build=String(data.commit).slice(0,8);if(data?.cache)window.JFM_RELEASE.serverCache=String(data.cache)}}catch{}render();requestCacheVersion();setTimeout(requestCacheVersion,300);emit()}
