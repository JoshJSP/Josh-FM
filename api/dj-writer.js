...
export default async function handler(req,res){
  ...
  try{
    const r=await timedFetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model,messages:[{role:'system',content:system},{role:'user',content:user}],temperature:.78,max_tokens:220,top_p:.9})},12000);
    const d=await r.json().catch(()=>({}));if(!r.ok)return res.status(r.status===429?429:502).json({error:d?.error?.message||`Groq HTTP ${r.status}`,provider:'groq',model});
    let text=String(d?.choices?.[0]?.message?.content||'').replace(/\s+/g,' ').replace(/^['"“”]+|['"“”]+$/g,'').trim();
    if(!text)return res.status(502).json({error:'Groq gaf geen DJ-tekst terug',provider:'groq',model});
    if(text.length>900)text=text.slice(0,900).replace(/\s+\S*$/,'').trim();
    return res.status(200).json({text,provider:'groq',model,usage:d?.usage||null});
  }catch(e){return res.status(504).json({error:e?.name==='AbortError'?'Groq timeout':String(e?.message||e),provider:'groq',model})}
}
