...
(()=>{
 ...
 // Force iOS to use a real PNG rather than the SVG fallback.
 document.querySelectorAll('link[rel="apple-touch-icon"]').forEach(x=>x.remove());
 const touch=document.createElement('link');touch.rel='apple-touch-icon';touch.sizes='512x512';touch.href='/api/icon?v=5';document.head.appendChild(touch);
 const fav=document.createElement('link');fav.rel='icon';fav.type='image/png';fav.href='/api/icon?v=5';document.head.appendChild(fav);
 ...
 if($('installHint')&&/iphone|ipad|ipod/i.test(navigator.userAgent))$('installHint').textContent='Op iPhone: verwijder de oude snelkoppeling, open Josh FM in Safari en kies Deel → Zet op beginscherm.';
