...
const targets=[
  ...
  ['.car-nav-maneuver strong',10,18,3],
...
function run(){queued=false;const root=document.querySelector(ROOT);if(!root)return;targets.forEach(([sel,min,max,maxLines])=>root.querySelectorAll(sel).forEach(el=>fit(el,min,max,maxLines)))}
...
window.MAIRCarAutoFit={version:'2026-08-31-v2-line-aware',refresh:schedule};
})();
