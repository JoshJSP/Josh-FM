...
(()=>{
  ...
  async function restoreReloadPlayback(){
    ...
    await api('/me/player/play?device_id='+encodeURIComponent(id),{method:'PUT',body:{uris:[intent.uri],position_ms:Math.min(Math.max(0,intent.progressMs+elapsed),Math.max(0,(intent.durationMs||Infinity)-1500))}});
    s=await verify(x=>x.device?.id===id&&x.is_playing&&x.item?.uri===intent.uri,10);if(!s)throw Error('Spotify bevestigde de track na vernieuwen niet.');await nudgeSdkPlayback();window.__jfmReloadContextUri=intent.uri;try{window.dispatchEvent(new CustomEvent('jfm:reload-context-restored',{detail:{uri:intent.uri}}))}catch{}reloadRestores++;reloadIntent=null;return{p,id,state:s}
  }
