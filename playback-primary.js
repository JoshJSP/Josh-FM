...
(()=>{
  ...
  // een keer gecontroleerd door mag gaan in plaats van op hetzelfde nummer te blijven hangen.
  const RESUME_GUARD_LIMIT=2;
  function noteResumeAttempt(trackId){trackId=String(trackId||'');if(!trackId)return 0;if(resumeGuardTrackId!==trackId){resumeGuardTrackId=trackId;resumeGuardAttempts=0}return ++resumeGuardAttempts}
  function clearResumeGuard(trackId=''){trackId=String(trackId||'');if(!trackId)return;if(resumeGuardTrackId&&resumeGuardTrackId!==trackId){resumeGuardTrackId='';resumeGuardAttempts=0}if(resumeGuardAdvancedId&&resumeGuardAdvancedId!==trackId)resumeGuardAdvancedId=''}
  ...
  async function recover(reason='watchdog'){if(backgrounded()||reloadNeedsGesture)return false;if(Date.now()<recoveryCooldownUntil||busy||endGuardBusy||djOwnsTransport())return false;const t=truth()?.get?.();if(!t?.expectedLive)return false;const reloadPending=!!reloadIntent,forceSync=/^(foreground-return|pageshow|visible|online)$/.test(String(reason));if(!reloadPending&&!forceSync&&truth()?.shouldRecover&&!truth().shouldRecover())return !!t.isPlaying;try{const{state:s}=reloadPending?await restoreReloadPlayback():await ensureActive({preserve:true});if(djOwnsTransport())return false;const ended=endedPlayback(s,t);if(ended)return handleNaturalEnd({...ended,source:`${ended.source}:${reason}`});if(s?.is_playing){ingest(s,reloadPending?'primary-reload-restored':'primary-recovery-already');recoveryFailures=0;recoveryCooldownUntil=0;return true}if(djOwnsTransport())return false;const stuckId=String(s?.item?.id||t.trackId||''),attempts=noteResumeAttempt(stuckId);const ok=await resume();if(ok){resumeGuardTrackId='';resumeGuardAttempts=0;recoveryFailures=0;recoveryCooldownUntil=0;recoveries++;info('Spotify-verbinding hersteld.');return true}/* Hervatten hielp niet. Na RESUME_GUARD_LIMIT pogingen op dezelfde track mag de radio exact een keer gecontroleerd doorgaan, nooit twee keer voor hetzelfde nummer. */if(stuckId&&attempts>=RESUME_GUARD_LIMIT&&resumeGuardAdvancedId!==stuckId&&!djOwnsTransport()){resumeGuardAdvancedId=stuckId;resumeGuardTrackId='';resumeGuardAttempts=0;info('Hetzelfde nummer bleef hangen. MAIR gaat een keer verder.');const advanced=await advance({record:false,source:'primary-resume-guard:'+reason});if(advanced){resumeGuardAdvances++;recoveries++;recoveryFailures=0;recoveryCooldownUntil=0;return true}}return ok}catch(e){if(e?.code==='RELOAD_GESTURE')return false;recoveryFailures++;if(recoveryFailures>=3){recoveryCooldownUntil=Date.now()+30000;recoveryFailures=0;info('Spotify-herstel wacht 30 seconden na meerdere fouten.',true)}return rememberError(e,'Playback-herstel mislukt: ')}}
