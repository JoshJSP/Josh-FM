...
(()=>{
  ...
  let sleepTimer=null,sleepState=loadSleep(),waitingWorker=null,wasOffline=!navigator.onLine,updateCheckTimer=null;
  ...
  function renderNetwork({initial=false}={}){
    if(!navigator.onLine){wasOffline=true;setConnectivityNote('Geen internet · de app blijft open, maar Spotify en Fish kunnen tijdelijk niet werken.','warn');return}
    if(wasOffline&&!initial){wasOffline=false;setConnectivityNote('Verbinding hersteld · Josh FM probeert verder te gaan.','info');return}
    wasOffline=false;if(initial){const x=ensureConnectivityBar();x?.classList.add('hidden')}
  }
  ...
  async function installServiceWorkerUX(){
    ...
    try{
      ...
      if(updateCheckTimer)clearInterval(updateCheckTimer);updateCheckTimer=setInterval(()=>checkForUpdate(reg),30*60*1000);
      window.addEventListener('pageshow',()=>setTimeout(()=>checkForUpdate(reg),1200));
      document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')setTimeout(()=>checkForUpdate(reg),800)});
    }catch{}
  }

  function install(){ensureStyles();installSleepCard();installMediaActions();scheduleSleep();renderSleep();renderNetwork({initial:true});updateMediaSession();installServiceWorkerUX()}
  ...
  window.JFMPWA={version:'pwa-v3-cold-start-safe',setSleepMinutes:setMinutes,stopAfterTrack,cancelSleep,get sleep(){return sleepState},updateMediaSession,showUpdate,setConnectivityNote,checkForUpdate};
})();
