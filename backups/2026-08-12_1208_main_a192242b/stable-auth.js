// Reliable Spotify PKCE for iPhone/PWA. Keeps the existing app.js token model, but makes the OAuth round-trip durable.
(()=>{
  ...
  syncSession();
  async function stableConnect(){
    const id=spotifyClientId||$('clientId')?.value.trim();
    if(!id)throw Error('Spotify Client ID ontbreekt.');
    ...
    localStorage.setItem(VER,verifier);localStorage.setItem(ST,state);
  ...
  async function reconcile(){
    syncSession();
    try{const t=await ensure();if(t){try{setConnected(true)}catch{};localStorage.removeItem(VER);localStorage.removeItem(ST);const q=$('queueInfo');if(q&&!/live|tracks klaar/i.test(q.textContent||''))q.textContent='Spotify gekoppeld. Josh FM is klaar om te starten.';return true}}
    catch{}
    return false;
  }
  setTimeout(reconcile,350);setTimeout(reconcile,1300);window.addEventListener('pageshow',()=>setTimeout(reconcile,250));window.JFMAuth={reconcile,connect:stableConnect};
})();