...
// Transport is owned by playback-primary.js and DJ handoff is owned by dj-handoff-v34.js.
(()=>{
  ...
  let player=null,deviceId='',initPromise=null;
  ...
  function syncIOSPosition(state,track){if(!('mediaSession'in navigator)||typeof navigator.mediaSession.setPositionState!=='function')return;try{const durationMs=Number(track?.duration_ms)||0,positionMs=Number(state?.position)||0;if(durationMs<=0)return;const duration=Math.max(.001,durationMs/1000),position=Math.max(0,Math.min(duration-.001,positionMs/1000));navigator.mediaSession.setPositionState({duration,playbackRate:1,position});navigator.mediaSession.playbackState=state?.paused?'paused':'playing'}catch{}}
  ...
  async function initPlayer(){
    ...
    initPromise=(async()=>{
      ...
      player=new Spotify.Player({name:'Josh FM',getOAuthToken:async cb=>{try{cb(await ensure()||'')}catch{cb('')}},volume:1,enableMediaSession:true});window.jfmSpotifyPlayer=player;
      const ready=new Promise((resolve,reject)=>{
        ...
        player.addListener('player_state_changed',state=>{
          ...
          playback=fake;try{renderPlayback(fake)}catch{};try{window.JFMPlaybackState?.ingest?.(fake,'sdk')}catch{};syncIOSPosition(state,t)
        })
      });
      const ok=await player.connect();if(!ok)throw Error('Spotify Web Player kon niet verbinden.');return ready
    })().catch(e=>{initPromise=null;rememberDevice('');message(e.message||String(e),true);throw e});
    return initPromise
  }
  ...
  window.JFMSpotifySDK={version:'sdk-core-v3-auth-device-only',init:initPlayer,transfer,playUris,get player(){return player},get deviceId(){return deviceId}};
})();
