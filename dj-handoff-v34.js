...
(()=>{
  ...
  let busy=false,restorePercent=100,restoreLocal=1,lastFailure='';
  ...
  const voiceReady=()=>typeof window.speakText==='function';
  ...
  async function buildSpeech(track,manual){if(!voiceReady())throw Error('Stemruntime is nog niet beschikbaar.');const[fact,weather]=await Promise.all([getFact(track),getWeather()]);const text=await makeDJScript(track,fact,weather,manual);if(!text)return null;try{if(typeof window.prepareSpeech==='function'){const ok=await window.prepareSpeech(text,false);if(ok===false)return null}}catch{}return{text,fact,weather}}
  async function speak(pack,manual){if(!pack?.text||!voiceReady())return false;if($('djText'))$('djText').textContent=pack.text;if($('factSource'))$('factSource').classList.add('hidden');if($('jingles')?.checked&&!manual&&Math.random()<.2)try{await window.speakText('MAIR.',true)}catch{}try{return(await window.speakText(pack.text,false))===true}catch{return false}}
  ...
  async function runBreak(track=null,manual=false){if(busy||sharedBusy())return false;busy=true;setSharedBusy(true);lastFailure='';let muteAttempted=false,muted=false,pausedForDJ=false,expectedUri='';try{
    ...
    if(!voiceReady())throw Error('Stemruntime is nog niet beschikbaar.');
    ...
    recordBreakTime();try{truth()?.patch?.({expectedLive:true,intent:'radio-live',progressMs:0},'dj-handoff-complete')}catch{};status('DJ klaar · nummer start vanaf het begin.');lastFailure='';return true
  }catch(e){lastFailure=String(e?.message||e||'DJ-handoff mislukt.');status(`DJ wacht · ${lastFailure}`,true);return false}finally{
  ...
  window.JFMDJTransition={version:'handoff-v37.1-voice-guard',iosFallback:'pause-speak-rewind-resume',transition:({track=null,manual=false}={})=>runBreak(track,manual),get busy(){return busy},get lastFailure(){return lastFailure}};
  window.JFMDJHandoff={version:'handoff-v37.1-voice-guard',runBreak,get busy(){return busy},get lastFailure(){return lastFailure},get lastBreakAt(){return Number(localStorage.getItem(LAST_BREAK_KEY)||0)}};
