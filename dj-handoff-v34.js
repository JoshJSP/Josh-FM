// Josh FM DJ handoff v36.1 — verified mute on iOS, then DJ, rewind, restore; automatic refresh delegates here.
(()=>{
  ...
  let busy=false,armed=null,polling=false,restorePercent=100,restoreLocal=1;
  ...
  async function setVolume(v){
    const p=player();
    if(isiOS()){
      const percent=v<=0?0:restorePercent;
      await api(pathWithDevice('/me/player/volume?volume_percent='+encodeURIComponent(percent)),{method:'PUT'});return
    }
    if(!p||typeof p.setVolume!=='function')throw Error('Josh FM-volumecontroller is niet klaar.');
    await p.setVolume(v<=0?0:restoreLocal)
  }
  async function confirmedMute(){
    if(isiOS()){
      for(let i=0;i<6;i++){await wait(120+i*80);const s=await currentState(),v=Number(s?.device?.volume_percent);if(Number.isFinite(v)&&v<=1)return true}return false
    }
    const p=player();if(typeof p?.getVolume!=='function')return false;for(let i=0;i<5;i++){await wait(80+i*60);try{if(Number(await p.getVolume())<=.01)return true}catch{}}return false
  }
  ...
  async function runBreak(track=null,manual=false){
    if(busy||sharedBusy())return false;busy=true;setSharedBusy(true);let muteAttempted=false,muted=false,expectedUri='';
    try{
      ...
      await rememberVolume(live);muteAttempted=true;await setVolume(0);
      if(!(await confirmedMute()))throw Error('DJ overgeslagen: iPhone-mute kon niet veilig worden bevestigd.');
      muted=true;try{truth()?.patch?.({expectedLive:true,intent:'dj-handoff'},'dj-handoff-v36-muted')}catch{};
      const target=track||(live?.item?trackObj(live.item):null);status('DJ live · muziek is bevestigd gedempt.');
      ...
      await setVolume(1);muteAttempted=false;muted=false;if(spoken)recordBreakTime();try{truth()?.patch?.({expectedLive:true,intent:'radio-live',progressMs:0},'dj-handoff-v36-complete')}catch{};
    ...
    }catch(e){status(String(e?.message||e),true);return false}
    finally{if(muteAttempted||muted)try{await setVolume(1)}catch{};setSharedBusy(false);busy=false}
  }
  ...
  function installRefreshDelegate(){
    try{
      refresh=async function(){
        const d=await api('/me/player');if(!d?.item)return;playback=d;try{window.JFMPlaybackState?.ingest?.(d,'app-refresh-v221')}catch{};renderPlayback(d);
        const id=d.item.id;if(lastTrackId&&id!==lastTrackId){const ended=session.find(x=>x.id===lastTrackId)||null;if(!session.some(x=>x.id===id)){session.unshift(trackObj(d.item));session=session.slice(0,12);renderHistory()}tracksSinceTalk++;
          if(tracksSinceTalk>=nextTalkAt&&!sharedBusy()){if(skipNextTalk){skipNextTalk=false;scheduleTalk()}else{await runBreak(ended,false);scheduleTalk()}}
        }else if(!lastTrackId&&!session.some(x=>x.id===id)){session.unshift(trackObj(d.item));renderHistory()}lastTrackId=id
      };return true
    }catch{return false}
  }
