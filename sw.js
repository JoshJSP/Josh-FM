const CACHE='josh-fm-v9-playback-truth';
const CORE=['./','./index.html','./styles.css','./app.js','./stability-core.js','./dj-resume.js','./discovery.js','./debug-tts.js','./director.js','./smart-dj.js','./radio-suite.js','./radio-upgrades.js','./playback-state.js','./spotify-recovery.js','./station-queue.js','./dj-now-queue.js','./manifest.webmanifest','./logo.svg'];
...
async function networkFirst(request){
  ...
  catch{const cached=await cache.match(request,{ignoreSearch:true});if(cached)return cached;if(request.mode==='navigate')return cache.match('./index.html')||cache.match('./');throw new Error('offline')}
}
