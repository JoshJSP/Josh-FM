...
const breakId=()=>String(dj?.activeBreak?.breakId||dj?.prepared?.breakId||dj?.lastHandoff?.breakId||lastEvent?.correlationId||'');
...
window.addEventListener('mair:dj-speaking',e=>{lastTTS=clean(e.detail||null);const d=e.detail||{},id=d.breakId||breakId();if(id)trace(id,d.active?'tts.playback-start':'tts.playback-end',!d.active&&d.success===false?'FAIL':'PASS',{provider:d.provider||'',route:d.route||'',playbackStartedAt:Number(d.playbackStartedAt)||0,playbackEndedAt:Number(d.playbackEndedAt)||0,success:d.success!==false,error:String(d.error||'')})});
...
window.MAIRObservability={version:'observability-v1.1-handoff-evidence',trace,snapshot,forBreak:id=>snapshot(String(id||'')),sanitize:clean,get breakId(){return breakId()}};
runtime()?.register?.('mair-observability',{version:'observability-v1.1-handoff-evidence',owner:'diagnostics'});
})();
