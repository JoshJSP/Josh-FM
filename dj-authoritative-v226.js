...
(()=>{
  ...
  async function run(manual=false){if(busy)return false;const live=await state();const uri=String(live?.item?.uri||'');if(!live?.is_playing||!uri.startsWith('spotify:track:'))return false;busy=true;let pausedForDJ=false;try{
    const track=trackObj(live.item);status('DJ wordt voorbereid…');const pack=await prepare(track,manual);if(!pack)return false;
    if(!(await pause(uri))){status('DJ overgeslagen · muziek kon niet veilig worden gepauzeerd.');return false}pausedForDJ=true;
    ...
    const same=await state();if(same?.item?.uri!==uri){status('DJ afgebroken · track is gewisseld.');return false}
  ...
  }catch(e){status(String(e?.message||e));return false}finally{if(pausedForDJ){const ok=await recover(uri).catch(()=>false);status(ok?'DJ afgebroken · muziek veilig hervat.':'DJ afgebroken · open Spotify om playback te hervatten.')}busy=false}}
  ...
  window.djBreak=()=>run(false);window.JFMDJAuthoritative={version:'v226-pause-speak-rewind-resume-recover',run,plan,get busy(){return busy}};
