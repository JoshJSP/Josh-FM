...
(()=>{
  ...
  async function run(manual=false){if(busy)return false;const live=await state();const uri=String(live?.item?.uri||'');if(!live?.is_playing||!uri.startsWith('spotify:track:')){lastFailure='blocked-not-playing';lastTrigger={at:Date.now(),manual,result:lastFailure,uri};renderSchedule();return false}if(!voiceReady()){lastFailure='voice-unavailable';lastTrigger={at:Date.now(),manual,result:lastFailure,uri};renderSchedule();status('DJ wacht · stemruntime is nog niet beschikbaar.');return false}busy=true;lastTrigger={at:Date.now(),manual,result:'preparing',uri};renderSchedule();let pausedForDJ=false;try{
