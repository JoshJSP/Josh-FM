...
(()=>{
  ...
  function ownButton(){const old=$('djNow');if(!old||old.dataset.jfmOwner==='v228')return;const b=old.cloneNode(true);old.replaceWith(b);b.dataset.jfmOwner='v228';b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();arm()},true)}
  ...
  window.refresh=async function(){const d=await api('/me/player');if(!d?.item)return;playback=d;try{window.JFMPlaybackState?.ingest?.(d,'dj-v228-refresh')}catch{};renderPlayback(d);await onTrackChange(d);lastTrackId=d.item.id};
  window.djBreak=()=>run(false);window.JFMDJAuthoritative={version:'v228-pending-break-delivery',run,plan,replan:replanFromSetting,state:scheduleState,retry:()=>attemptPending('manual-retry'),get busy(){return busy}};
