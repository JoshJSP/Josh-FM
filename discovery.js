...
window.buildSet=buildSet=async function(){
  ...
  let ideas=[];try{const r=await boundedFetch('/api/discover',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({seeds:seedRows,count:Math.min(6,wanted+2),mode:settings?.mode||'normal'})});if(r.ok){const d=await r.json().catch(()=>({}));ideas=Array.isArray(d.tracks)?d.tracks.slice(0,4):[];stats.aiIdeas=ideas.length}}catch(e){stats.errors++;stats.lastError=e?.name==='AbortError'?'Discovery API timeout':String(e?.message||e)}
