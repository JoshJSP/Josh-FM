...
async function rebuildDiscoveryStation(force=false){if(!isDiscoveryStation())return false;if(busy&&!force)return false;const token=++buildToken;busy=true;const v=clamp(slider.value),info=document.getElementById('queueInfo');try{if(info)info.textContent=`MAIR Discovery · ${levelName(v)} · nieuwe muziek zoeken…`;const existing=(typeof queue!=='undefined'&&Array.isArray(queue)?queue:[]).map(t=>({...t})),found=[...existing];for(const q of queryPlan(v)){if(token!==buildToken||!isDiscoveryStation())return false;found.push(...await search(q))}if(token!==buildToken||!isDiscoveryStation())return false;let ranked=dedupe(found).filter(t=>{const y=Number(String(t.release||'').slice(0,4));return!y||y>=NOW()-1}).map(t=>t._discovery===true?{...t,_discoveryReason:t._discoveryReason||`MAIR Discovery · ${levelName(v)}`}:{...t}).sort((a,b)=>adventureScore(b,v)-adventureScore(a,v));if(ranked.length<5)return false;const currentId=(()=>{try{return playback?.item?.id||''}catch{return''}})(),currentTrack=currentId?existing.find(t=>t.id===currentId):null;if(currentTrack)ranked=[currentTrack,...ranked.filter(t=>t.id!==currentId)];queue=ranked.slice(0,50);try{window.__jfmStationQueueSig=''}catch{};try{window.jfmRenderNext?.();window.JFMProgramDirector?.render?.()}catch{}if(info)info.textContent=`${queue.length} tracks klaar · MAIR Discovery · ${v}% ${levelName(v)}.`;try{window.dispatchEvent(new CustomEvent('mair:discovery-station-refreshed',{detail:{percent:v,level:levelName(v),tracks:queue.length}}))}catch{}return true}finally{busy=false}}
