...
(()=>{
  ...
  function loadSDK(){if(window.Spotify)return Promise.resolve();return new Promise((resolve,reject)=>{const old=window.onSpotifyWebPlaybackSDKReady;window.onSpotifyWebPlaybackSDKReady=()=>{try{old?.()}catch{}resolve()};let s=document.getElementById('spotify-sdk-stable');if(s){const timer=setInterval(()=>{if(window.Spotify){clearInterval(timer);resolve()}},100);setTimeout(()=>{clearInterval(timer);if(!window.Spotify)reject(Error('Spotify-speler kon niet laden.'))},SDK_LOAD_TIMEOUT_MS);return}s=document.createElement('script');s.id='spotify-sdk-stable';s.src='https://sdk.scdn.co/spotify-player.js';s.async=true;s.onerror=()=>reject(Error('Spotify-speler kon niet laden.'));document.head.appendChild(s)})}
