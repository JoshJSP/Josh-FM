...
(()=>{
  ...
  const POOL_CACHE='mair_station_pool_cache_v3_spotify_2026',CACHE_TTL=12*60*60*1000;
  ...
  async function buildPool(id){
    ...
    if(filtered.length<minimum&&['hits','top40','new'].includes(id)&&raw.length>=minimum){filtered=raw;fallback=true}
  ...
  async function choose(id){
    ...
    try{
      status(`${stationLabel(id)} zoekt muziek op Spotify…`);
  ...
  window.MAIRStationController={version:'mair-station-controller-v3.7-spotify-2026',select:choose,harden:own,buildPool,get switching(){return switching},get channel(){return localStorage.getItem('jfm_music_channel_v1')||'mix'}};window.JFMChannelTapGuard=window.MAIRStationController;
})();