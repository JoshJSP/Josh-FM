(()=>{
  ...
  // Transformers.js currently assumes tokenizer.json for this raw checkpoint, so
  // Josh FM builds the tiny MMS character tokenizer locally and loads the model.
  ...
  async function hf(path){const r=await fetch(`https://huggingface.co/${MODEL}/resolve/main/${path}`);if(!r.ok)throw new Error(`${path} kon niet worden geladen (HTTP ${r.status})`);return r.json()}
  function makeTokenizer(vocab){
    ...
    return text=>{
      // MMS/VITS vocab is character based. Normalize like the Python tokenizer's basic path.
  ...
  async function load(){
    ...
    state.loading=(async()=>{
      ...
      let model=null,last=null;for(const device of (navigator.gpu?['webgpu','wasm']:['wasm'])){try{model=await AutoModel.from_pretrained(MODEL,{device,dtype:'fp32'});break}catch(e){last=e}}
  ...
  async function capabilities(){let cache=false;try{cache=!!window.caches}catch{}const report={engine:'MMS neural TTS',model:MODEL,webgpu:!!navigator.gpu,wasm:typeof WebAssembly==='object',cache,loaded:!!state.pipe,error:state.error};state.lastReport=report;return report}
