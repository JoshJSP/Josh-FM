...
function enforceNotLive(detail={}){if(window.MAIRVoiceEngine?.speaking)return;setLive(false);status(false);truthfulTalkStatus(detail)}
window.addEventListener('mair:dj-v2-state',e=>{if(!window.MAIRDJ?.version?.startsWith('v3')||window.MAIRVoiceEngine?.speaking)return;const detail=e.detail||{};enforceNotLive(detail);setTimeout(()=>enforceNotLive(detail),0)});
...
window.MAIRDJLiveTruth={version:'mair-dj-live-truth-v1.3-post-render-enforced',setLive,truthfulTalkStatus,enforceNotLive};
})();
