...
(()=>{
  ...
  function rows(){
    ...
    return [
      ...
      ['Playback-fouten',String(health.failures??0)],['Herstelacties',String(health.recoveries??0)],['Device-handoffs',String(health.deviceHandovers??0)],['Exacte reload-restores',String(health.reloadRestores??0)],['Reload wacht op tik',health.reloadNeedsGesture?'ja · browserautoplay geblokkeerd':'nee'],['Herstel-backoff',health.recoveryCooldownMs?`${Math.ceil(health.recoveryCooldownMs/1000)} sec`:'geen'],['Queue-bron',queue.station?`${queue.station} · ${queue.source||'onbekend'}`:'niet geladen'],['Queue-revisie',String(queue.revision??'—')],['Spotify-queue sync',upcoming?.lastOk?`${Math.max(0,Math.round((Date.now()-upcoming.lastOk)/1000))} sec geleden`:'nog niet'],
      ['DJ-fase',dj.phase?`${dj.phase} · ${dj.reason||'geen reden'}`:'niet geladen'],['DJ volgende beslissing',Number.isFinite(dj.remaining)?`over ${dj.remaining} nummer${dj.remaining===1?'':'s'}`:'onbekend'],['DJ-overgangen',djSync.lastTransition?`${djSync.lastTransition.cause} · ${djSync.lastTransition.pair} · ${Math.max(0,Math.round((Date.now()-djSync.lastTransition.at)/1000))} sec geleden`:'nog geen bewezen overgang'],['DJ synthetische signalen',String(djSync.syntheticSignals??0)],['Actieve break',dj.activeBreak?`${dj.activeBreak.breakId} · ${dj.activeBreak.status}`:'geen'],['DJ-writer',dj.writer?`${dj.writer.provider||'onbekend'} · ${dj.writer.model||'geen model'}${dj.writer.error?' · fallback na fout':''}`:'nog niet gebruikt'],['DJ-stem',audio.voiceTitle||audio.voiceId||dj.voice?.voiceId||'nog niet bevestigd'],['TTS-status',audio.error?`fout · ${audio.error}`:audio.cacheSize?`${audio.cacheSize} break(s) voorbereid`:audio.lastPlaybackAt?`laatst afgespeeld ${Math.max(0,Math.round((Date.now()-audio.lastPlaybackAt)/1000))} sec geleden`:'nog niet gebruikt'],['TTS-route',audio.playbackMode||'nog niet gebruikt'],['Laatste DJ-fout',dj.error||dj.lastMissReason||'geen'],['Laatste herstel',lastRepair||'nog niet']
    ];
  }
  function render(){
    ensureUi();
