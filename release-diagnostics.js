...
(()=>{
  ...
  function rows(){
    ...
    return [
      ...
      ['Playback-fouten',String(health.failures??0)],['Herstelacties',String(health.recoveries??0)],['Device-handoffs',String(health.deviceHandovers??0)],['Herstel-backoff',health.recoveryCooldownMs?`${Math.ceil(health.recoveryCooldownMs/1000)} sec`:'geen'],['Queue-bron',queue.station?`${queue.station} · ${queue.source||'onbekend'}`:'niet geladen'],['Queue-revisie',String(queue.revision??'—')],['Spotify-queue sync',upcoming?.lastOk?`${Math.max(0,Math.round((Date.now()-upcoming.lastOk)/1000))} sec geleden`:'nog niet'],
