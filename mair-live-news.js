...
function render(){const p=$('mairNewsStatus');if(!p)return;const s=peek();p.textContent=!s.enabled?'Nieuwscontext staat uit.':s.error?`Nieuws tijdelijk niet beschikbaar · ${s.error}`:s.headline?`Volgend halfuurmoment · ${s.source} · ${s.headline.title}`:'Geen nieuwe headline voor het volgende halfuurmoment.'}
...
window.MAIRLiveNews={version:'mair-live-news-v1.2-half-hour-no-repeat',refresh,peek,take,setEnabled,get enabled(){return isEnabled()}};
})();