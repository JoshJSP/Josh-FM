// MAIR Live News — opt-in NOS context for :30 headlines and hourly bulletins.
(()=>{
'use strict';
...
function installUI(){
  ...
  // Niet bouwen is eerlijker dan verbergen.
  if(window.MAIR_DJ_ENABLED!==true)return;
  if($('newsMention'))return;const weather=$('weatherMention'),host=weather?.closest('label');if(!host)return;const label=document.createElement('label');label.className='switch';label.innerHTML='<input id="newsMention" type="checkbox"><span></span><b>MAIR Nieuws (:00) + headlines (:30)</b>';host.insertAdjacentElement('afterend',label);const p=document.createElement('p');p.id='mairNewsStatus';p.className='muted';p.style.marginTop='8px';label.insertAdjacentElement('afterend',p);$('newsMention').checked=isEnabled();$('newsMention').addEventListener('change',e=>setEnabled(e.target.checked));render()}
function render(){const p=$('mairNewsStatus');if(!p)return;const s=peek();p.textContent=!s.enabled?'Nieuws staat uit.':s.error?`Nieuws tijdelijk niet beschikbaar · ${s.error}`:!s.feedFresh?'Nieuwsfeed wordt vernieuwd voordat er iets wordt uitgezonden.':s.headline?`Ieder heel uur MAIR Nieuws · om :30 korte headlines · ${s.source}`:`Ieder heel uur MAIR Nieuws · om :30 korte headlines · ${s.source}`}
function boot(){installUI();data=cached();if(isEnabled())refresh(true);setInterval(()=>refresh(false),8*60*1000);setInterval(render,30000)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.MAIRLiveNews={version:'mair-live-news-v2-hourly-bulletin-half-headlines',refresh,peek,take,bulletinItems,feedFresh,setEnabled,get enabled(){return isEnabled()}};
})();
