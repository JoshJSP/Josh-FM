...
(()=>{
  ...
  function ensure(){
    ...
    // Existing session/history UI is intentionally removed from the product surface.
  ...
  ['jfm:playback-state','jfm:trackchange','jfm:requests-change','jfm:clock-tick','jfm:show-change','jfm:dj-audio-health'].forEach(name=>window.addEventListener(name,render));
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)render()});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',ensure);else ensure();
  setInterval(render,2500);
  window.JFMLiveUI={version:'live-ui-v1',render};
})();
